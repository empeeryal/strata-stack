...
│   │   ├── react/           # React islands (command palette, contact form, auth and account forms)
...
`.netlify/`, `.wrangler/`, `coverage/`, `playwright-report/`, `test-results/` and the database
files in `.data/` (the directory itself is tracked through `.data/.gitkeep`).
