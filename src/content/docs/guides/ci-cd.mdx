...
description: The GitHub Actions workflows, the build matrix for every deploy target, Dependabot and Changesets releases.
...
development tooling, which never runs in production) are listed in `pnpm.auditConfig.ignoreGhsas`
in `package.json`. Patchable transitive packages are pinned through `pnpm.overrides`. Review both
lists when upgrading dependencies.
...
passes the same gate to pnpm while it rewrites the lockfile, and pnpm 10 applies it to every
package it re-resolves, including ones already pinned in the lockfile. An update therefore fails
with `ERR_PNPM_NO_MATURE_MATCHING_VERSION` while any package in `pnpm-lock.yaml` is younger than
three days, which happens after a manual `pnpm install` picked up fresh versions. The next run
succeeds once those packages have aged.
