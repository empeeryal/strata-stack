...
a file under `drizzle/`. It reads `DATABASE_URL` and `DATABASE_AUTH_TOKEN` from repository
secrets (or from a `production` environment, which the job references, so you can add a
required-reviewer rule there) and refuses a `file:` URL.
