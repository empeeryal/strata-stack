...
The middleware also runs `assertProductionConfig()` once per server instance: a missing or
short `BETTER_AUTH_SECRET` stops on-demand requests with a clear error, and a missing email
provider or contact recipient is logged as a warning. `/api/health` reports the same facts to
administrators and to monitors that send `Authorization: Bearer <HEALTH_TOKEN>`.
...
Better Auth ships plugins for two-factor authentication, organizations, passkeys and more. Add
